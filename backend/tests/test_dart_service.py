...
class TestFetchDisclosuresForTickers:
    @pytest.mark.asyncio
    async def test_empty_api_key_returns_empty(self, override_settings):
        from app.services.dart_service import fetch_disclosures_for_tickers

        result = await fetch_disclosures_for_tickers(["005930"], "")
        assert result == []
    ...
    async def test_empty_tickers_returns_empty(self, override_settings):
        ...
        result = await fetch_disclosures_for_tickers([], "key")
        assert result == []

    @pytest.mark.asyncio
    async def test_ticker_not_in_corp_map_returns_empty(self, override_settings):
        import app.services.dart_service as ds

        with patch.object(ds, "_ensure_corp_code_map", new=AsyncMock(return_value={})):
            result = await ds.fetch_disclosures_for_tickers(["999999"], "key")

        assert result == []

    @pytest.mark.asyncio
    async def test_returns_disclosures_sorted_by_date(self, override_settings):
        import app.services.dart_service as ds
        ...
        assert "dart.fss.or.kr" in result[0]["dart_url"]

    @pytest.mark.asyncio
    async def test_http_error_returns_empty_for_that_ticker(self, override_settings):
        import app.services.dart_service as ds
    ...
    @pytest.mark.asyncio
    async def test_non_000_status_returns_empty(self, override_settings):
        import app.services.dart_service as ds
