...
def _patch_common(mock_db, composite_level="GREEN"):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch("app.services.rebalancing.plan_notifications.save_alert_history", new=AsyncMock()))
...
class TestRunAutoExecution:
    ...
    async def test_market_signal_failure_defaults_to_green_and_continues(self):
        ...
        with (
            ...
            patch("app.services.rebalancing.plan_notifications.save_alert_history", new=AsyncMock()),
    ...
    @pytest.mark.asyncio
    async def test_plan_generation_in_progress_skips_silently_and_continues(self):
        """build_pending_plan_for_alert()가 PlanGenerationInProgress를 반환하면(수동 "지금 실행"과
        거의 동시에 겹친 락 경합) 알림 없이 조용히 다음 alert로 넘어가야 한다 — 다음 5분 tick에
        자연히 재시도되므로 사용자에게 알릴 내용이 없다."""
        ...
        with (
            ...
            patch("app.jobs.rebalancing_auto_execution.notify_plan_generated", new=AsyncMock()) as mock_notify,
        ):
            from app.jobs.rebalancing_auto_execution import _run_auto_execution
    ...
    async def test_generates_plan_and_saves_history_when_drifting(self):
        ...
        with (
            ...
            patch("app.services.rebalancing.plan_notifications.save_alert_history", new=AsyncMock()) as mock_save,
...
class TestBuildPendingPlanForAlert:
    ...
    async def test_returns_none_when_no_items_exceed_threshold(self):
        ...
        with (
            ...
            patch(
                "app.services.rebalancing.plan_generation.generate_pending_plan_for_alert", new=AsyncMock()
            ) as mock_gen,
        ):
    ...
    async def test_daily_value_cap_unset_skips_check_entirely(self):
        ...
        with (
            ...
            patch("app.services.rebalancing.plan_generation.refresh_live_prices", new=AsyncMock()),
            patch(
                "app.services.rebalancing.plan_generation.sum_today_auto_plan_value_krw", new=AsyncMock()
            ) as mock_sum,
            patch(
                "app.services.rebalancing.plan_generation.generate_pending_plan_for_alert",
                new=AsyncMock(return_value=(plan, "buy-token", None)),
