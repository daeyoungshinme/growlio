...
class TestGetSyncAllStatus:
    @pytest.mark.asyncio
    async def test_returns_idle_when_no_record(self, make_user_id, mock_redis):
        from app.services.sync_all_service import get_sync_all_status

        mock_redis.get = AsyncMock(return_value=None)
        with patch("app.services.sync_all_service.get_redis", AsyncMock(return_value=mock_redis)):
            status = await get_sync_all_status(make_user_id)

        assert status == {"status": "idle"}

    @pytest.mark.asyncio
    async def test_returns_stored_status(self, make_user_id, mock_redis):
        import json
...
class TestIsSyncAllRunning:
    @pytest.mark.asyncio
    async def test_false_when_no_lock(self, make_user_id, mock_redis):
    ...
    @pytest.mark.asyncio
    async def test_true_when_lock_present(self, make_user_id, mock_redis):
        from app.services.sync_all_service import is_sync_all_running

        mock_redis.get = AsyncMock(return_value="some-lock-value")
        with patch("app.services.sync_all_service.get_redis", AsyncMock(return_value=mock_redis)):
...
class TestRunSyncAll:
    @pytest.mark.asyncio
    async def test_skips_when_lock_already_held(self, make_user_id, make_account, mock_redis):
        """이미 락이 걸려 있으면 _sync_accounts를 호출하지 않고 조용히 반환한다."""
        from app.services.sync_all_service import run_sync_all

        mock_redis.set = AsyncMock(return_value=None)  # SET NX 실패 → 락 획득 실패
        accounts = [make_account()]

        with (
            patch("app.services.sync_all_service.get_redis", AsyncMock(return_value=mock_redis)),
    ...
    @pytest.mark.asyncio
    async def test_runs_batch_sync_and_marks_done(self, make_user_id, make_account, mock_redis):
        """락 획득에 성공하면 _sync_accounts를 실행하고 상태를 done으로 남긴다."""
        from app.services.sync_all_service import run_sync_all

        mock_redis.set = AsyncMock(return_value=True)  # SET NX 성공 → 락 획득
        mock_redis.get = AsyncMock(return_value="lock-value")  # redis_lock 해제 시 자기 락인지 확인용
    ...
    @pytest.mark.asyncio
    async def test_marks_error_status_when_sync_raises(self, make_user_id, make_account, mock_redis):
        """_sync_accounts가 예외를 던지면 status를 error로 남기고 예외를 흡수한다."""
        from app.services.sync_all_service import run_sync_all

        mock_redis.set = AsyncMock(return_value=True)
        mock_redis.get = AsyncMock(return_value="lock-value")
        accounts = [make_account()]
        ...
        last_call = mock_redis.setex.await_args_list[-1]
        import json
