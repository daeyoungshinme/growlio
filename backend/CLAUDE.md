...
> 파일명 접미사 컨벤션: `*_service.py`(DB/외부 API 연동 포함 유스케이스), `*_calculator.py`/`*_aggregator.py`(순수 계산·집계, 부수효과 없음), 접미사 없는 파일(`yahoo_price.py`, `backtest_metrics.py` 등)은 특정 도메인 유틸 모음. 강제 통일 대상 아님 — 새 파일 추가 시 참고용.
  ...
  │   ├── plan_service.py     # AUTO 리밸런싱 2단계 플랜(계획 생성 → 매수 대기/매도 승인 → 실행) 생명주기 관리 — 매수는 대기시간 경과 후 자동 실행(취소 가능), 매도는 이메일 승인 필요(당일 장마감 미응답 시 자동 만료). 토큰은 SHA-256 해시만 저장, `FOR UPDATE`로 중복 실행 방지 (구 rebalancing_plan_service.py). `build_pending_plan_for_alert()`가 세금영향(`RebalancingAlert.tax_impact_gate_mode`)·시장신호·유저 단위 하루 합산 거래한도(`UserSettings.auto_rebalancing_daily_value_cap_krw`, nullable=무제한) 게이트로 계획 생성을 막으면 각각 `TaxGateBlocked`/`MarketSignalGateBlocked`/`DailyValueCapBlocked` sentinel을 반환(플랜 미생성) — `notify_tax_gate_blocked()`/`notify_market_signal_gate_blocked()`/`notify_daily_value_cap_blocked()`가 하루 1회(Postgres 기반 durable_state dedup — 재시작에도 유지) 이메일/푸시/이력으로 보류 사유를 안내(앞 둘은 알림 단위, 하루 합산 캡은 게이트 자체가 유저 단위라 dedup도 user_id 기준). 하루 합산 캡 판정은 `sum_today_auto_plan_value_krw()`(오늘 KST 기준 PENDING/EXECUTED leg 합산)와 드리프트 항목의 `diff_krw` 절대값 합(클램프 이전 추정치, 보수적) 비교로 이뤄진다. `execute_due_buy_legs()`는 대기시간이 지난 매수 leg 실행 직전 시장신호 게이트를 재확인(계획 생성 시점 이후 상황 악화 대응, 차단되면 조용히 다음 1분 tick에 재시도). leg 실행 자체가 예외로 실패하면(개별 종목 주문 실패와는 별개) `_notify_leg_execution_failed()`가 이메일/푸시로 안내
  ...
  ├── goal_recommendation_service.py  # 목표 역산 포트폴리오 추천 API 진입점 3종(전체 자산 기준/투자기간별/연령대별) — 목표금액/월적립액/목표연도 → 필요 수익률 역산 → MVO 최적화 호출로 최소분산 포트폴리오 추천. 배당 목표(`annual_dividend_goal`)가 있으면 필요 배당수익률도 제약으로 함께 전달(전체 자산 기준과 동일한 %를 투자기간별 경로 모든 조합에도 동일 적용, 큐레이션 후보로 달성 불가하면 fail-soft로 무시). `get_age_based_recommendation()`(연령대별)은 목표 역산 없이 사용자가 직접 선택한 `UserSettings.age_group`(20대/30대/40대/50대/60대 이상)을 `_AGE_GROUP_PROFILE`로 조회해 risk_tolerance + 주식비중 상/하한 + 연령대 기본 배당수익률 하한(20~30대 0%, 40대 1.5%, 50대 2.5%, 60대 이상 3.5%)으로 추천을 계산 — 투자기간별과 마찬가지로 `_NON_BINDING_RETURN_FLOOR`로 필요수익률 제약을 무효화. `annual_dividend_goal`이 명시적으로 설정돼 있으면 연령대 기본값 대신 그 목표에서 계산된 배당수익률이 우선 적용된다(fail-soft는 다른 경로와 동일). `_suggest_for_dividend_goal()`은 등록된 후보만으로 배당 목표 달성이 어려우면 큐레이션 유니버스(`recommendation_universe.py`)에서 배당수익률이 높은 미등록 후보를 응답의 `suggested_candidates` 필드로 제안한다(전체 자산 기준/투자기간별/연령대별 3경로 모두 호출) — DB에 쓰지 않으며 이번 추천 계산(`recommended_items`)에도 반영하지 않는다. 사용자가 추천 카드의 "후보에 추가" 버튼으로 승인해야만 `PUT /settings/goal-candidate-tickers`(기존 전체교체 엔드포인트)를 통해 저장되고, 그래야 다음 추천부터 비중 계산에 포함된다 — `_get_or_seed_candidates()`가 최초 1회만 시딩하고 이후 자동 병합이 없어(의도된 설계) 큐레이션 유니버스에 새 ETF를 추가해도 기존 후보 등록 사용자에게는 노출되지 않는 gap을, 사용자 동의 없이 후보 목록을 바꾸지 않으면서 메운다. `compute_portfolio_expected_metrics()`는 포트폴리오의 현재 목표 비중에 대해 추천과 동일한 지표(기대수익률/배당수익률/변동성)를 계산(적용 전 비교 미리보기 전용). `compute_recommendation_drift()`는 프론트 `recommendationDrift.ts`와 동일한 드리프트 계산의 백엔드 포팅(주간 알림 job 전용). 자동 반영 안 됨 — 사용자가 확인 후 수동 적용
  ├── goal_portfolio_optimizer.py  # 목표 역산 추천 전용 MVO 최적화 엔진(SLSQP, DB 의존 없는 순수 계산) — goal_recommendation_service.py 서브모듈. 기대수익률·공분산은 `estimation.py`의 축소추정(shrinkage)을 거친 값을 사용(표본 추정치 그대로 쓸 때의 과최적화 완화). 자산군 단위 비중 제약은 `asset_classes`(종목별 EQUITY/BOND/CASH 태그)+`class_bounds`(자산군별 (하한,상한) 비율)로 N개 자산군을 일반화 지원 — 과거의 EQUITY vs 비EQUITY 이분법(`is_equity`+`equity_floor`/`equity_ceiling`)은 `class_bounds={"EQUITY": (floor, 1.0)}` 형태로 호출측(단기/IRP/연령대별)이 매핑. 배당 목표 달성가능성 사전검증(`_dividend_floor_constraint`)은 `class_bounds`가 걸려 있으면 그 그룹 예산도 함께 반영한다 — 종목당 상한만 보면 자산군 하한이 저배당 종목에 비중을 강제 배분시켜 실제로는 불가능한데도 "달성 가능"으로 오판, SLSQP 전체가 실패(빈 추천)하는 버그가 있었다(단기·IRP 추천에서 재현). `compute_weighted_expected_metrics()`는 이미 정해진(최적화 대상 아닌) 비중에 대해 가중평균 기대수익률/배당수익률/변동성만 계산하는 버전
