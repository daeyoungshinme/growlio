...
async def portfolio_summary(
    ...
    if not settings_row or not settings_row.kis_app_key:
        raise HTTPException(status_code=400, detail="KIS 설정이 필요합니다")
    ...
    domestic = await get_domestic_balance(app_key, app_secret, access_token, settings_row.kis_account_no, is_mock=is_mock)
    overseas = await get_overseas_balance(app_key, app_secret, access_token, settings_row.kis_account_no, is_mock=is_mock)

    stock_return_pct = 0.0
    if domestic["invested_krw"] > 0:
        stock_return_pct = (domestic["total_value_krw"] / domestic["invested_krw"] - 1) * 100

    return {
        "domestic": domestic,
        "overseas": overseas,
        "total_value_krw": domestic["total_value_krw"],
        "total_invested_krw": domestic["invested_krw"],
        "unrealized_pnl_krw": domestic["pnl_krw"],
