...
TTL_DCA_ANALYSIS = 900  # DCA 분석 응답 15분 (대시보드와 동일 무효화 트리거 공유)
...
def dca_analysis_key(user_id: uuid.UUID) -> str:
    return f"{_env_prefix()}dca_analysis:{user_id}"
...
def rebalancing_analysis_key(
    user_id: uuid.UUID,
    portfolio_id: uuid.UUID | str,
    acct_suffix: str,
    deposit_override: float | None,
) -> str:
    override_part = "none" if deposit_override is None else str(deposit_override)
