...
_MAX_WEIGHT = 0.4  # 종목당 최대 비중 상한 (과도한 집중 방지)
_MIN_RETURN_DAYS = 30
_MIN_CANDIDATES = 2
...
def _optimize_goal_portfolio(
    ...
    """분산 최소화 + 목표수익률 이상 제약(SLSQP). (recommended_items, expected_return_pct, note) 반환. 동기 함수."""
    import numpy as np
    ...
    max_weight = max(_MAX_WEIGHT, 1.0 / n)  # n이 작아 상한 합이 100%를 못 채우면 완화
...
async def get_goal_recommendation(
    ...
    cagr_map, dividend_map = await asyncio.gather(
        get_historical_returns(tickers_only, redis=redis),
    ...
    items, expected_return_pct, opt_note = await loop.run_in_executor(
        None, _optimize_goal_portfolio, f_symbols, f_tickers, f_cagrs, returns_map, required_return_pct
    )
