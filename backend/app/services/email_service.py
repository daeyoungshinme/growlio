...
async def send_rebalancing_plan_pending_email(
    ...
    buy_items: list,
    buy_deadline_at,
    buy_cancel_token: str | None,
    sell_items: list,
    sell_deadline_at,
    sell_action_token: str | None,
) -> bool:
    """AUTO 모드 플랜 생성 직후 발송하는 실행 전 계획 안내 이메일. 발송 성공 시 True.

    매수/매도 leg는 서로 독립적으로 존재할 수 있다 — `buy_cancel_token`이 None이면(매도 전용
    플랜) 매수 섹션 없이 매도 안내만 담아 발송한다.
    """
    if not _email_configured():
        logger.warning("email_not_configured_skip_email", to=to_email)
        return False
    buy_cancel_link = (
        f"{settings.frontend_url}/rebalancing/plan-confirm?token={buy_cancel_token}" if buy_cancel_token else None
    )
    sell_action_link = (
        f"{settings.frontend_url}/rebalancing/plan-confirm?token={sell_action_token}" if sell_action_token else None
    )
