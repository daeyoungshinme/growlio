...
export default function RebalancingStrategyCard({ portfolioId, portfolioName }: Props) {
  ...
  const { factor_changes, frontier_changes, trade_recommendations, overall_direction, summary } =
    data;
  const hasFrontier = frontier_changes.current_risk != null && frontier_changes.target_risk != null;
  ...
  return (
    <div className="card space-y-5">
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
            현재 포트폴리오 →{" "}
            <span className="text-amber-600 dark:text-amber-400 font-medium">{portfolioName}</span>
          </p>
        </div>
            ...
            <div className="space-y-2">
              <div className="rounded-lg bg-gray-50 dark:bg-gray-700/50 p-2.5 text-xs">
                ...
                <p className="font-semibold text-gray-700 dark:text-gray-300">
                  {frontier_changes.current_risk!.toFixed(2)}% →{" "}
                  {frontier_changes.target_risk!.toFixed(2)}%
                </p>
                <p
              ...
              <div className="rounded-lg bg-gray-50 dark:bg-gray-700/50 p-2.5 text-xs">
                ...
                <p className="font-semibold text-gray-700 dark:text-gray-300">
                  ...
                  {frontier_changes.current_return!.toFixed(2)}% →{" "}
                  {frontier_changes.target_return! >= 0 ? "+" : ""}
              ...
              {frontier_changes.sharpe_improvement != null && (
                <div className="rounded-lg bg-gray-50 dark:bg-gray-700/50 p-2.5 text-xs">
                  ...
                  <p
                    className={`font-semibold ${frontier_changes.sharpe_improvement ? "text-red-500" : "text-blue-500"}`}
                  >
                    {frontier_changes.sharpe_improvement ? "✓ 개선" : "✗ 하락"}
                  </p>
                  {frontier_changes.current_sharpe != null &&
                    frontier_changes.target_sharpe != null && (
                      <p className="text-gray-400 dark:text-gray-500 mt-0.5">
                        {frontier_changes.current_sharpe.toFixed(2)} →{" "}
                        {frontier_changes.target_sharpe.toFixed(2)}
                      </p>
                    )}
                </div>
              )}
      ...
      {trade_recommendations.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-gray-600 dark:text-gray-400">종목별 조정 방향</p>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700">
                  ...
                  <th className="text-center py-1.5 px-1 font-medium text-gray-400 dark:text-gray-500">
                    방향
                  </th>
                  ...
                  <th className="text-left py-1.5 pl-2 font-medium text-gray-400 dark:text-gray-500">
                    사유
                  </th>
                </tr>
              </thead>
