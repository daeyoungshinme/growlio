...
import { fetchGoalRecommendation, fetchOverallGoalRecommendation } from "@/api/rebalancing";
import { QUERY_KEYS } from "@/constants/queryKeys";
import { STALE_TIME } from "@/constants/queryConfig";
import { usePendingRecommendationStore } from "@/stores/pendingRecommendationStore";

interface Props {
  /** 특정 포트폴리오 기준 추천. 생략 시 전체 계좌(전체 투자현황) 기준 추천으로 동작하며,
   * 적용할 단일 대상 포트폴리오가 없으므로 "적용" 버튼은 표시하지 않는다(읽기 전용). */
  portfolioId?: string;
...
export default function GoalRecommendationCard({ portfolioId, noTopMargin = false }: Props) {
  const setPending = usePendingRecommendationStore((s) => s.setPending);

  const { data } = useQuery({
    queryKey: portfolioId
      ? QUERY_KEYS.goalRecommendation(portfolioId)
      : QUERY_KEYS.goalRecommendationOverall,
    queryFn: () =>
      portfolioId ? fetchGoalRecommendation(portfolioId) : fetchOverallGoalRecommendation(),
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-purple-700 dark:text-purple-400">
          목표 달성 추천 비중{!portfolioId && " (전체 자산 기준)"}
        </span>
      </div>
