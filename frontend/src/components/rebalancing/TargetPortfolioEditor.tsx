...
import { TargetPortfolio, TargetPortfolioItem } from "../../api/rebalancing";

interface Props {
  initial?: TargetPortfolio | null;
  onSave: (name: string, items: TargetPortfolioItem[], baseType: string) => void;
  onClose: () => void;
  saving?: boolean;
}

const EMPTY_ITEM: TargetPortfolioItem = { ticker: "", name: "", market: "KOSPI", weight: 0 };

export default function TargetPortfolioEditor({ initial, onSave, onClose, saving }: Props) {
  const [name, setName] = useState(initial?.name ?? "");
  const [baseType, setBaseType] = useState(initial?.base_type ?? "STOCK_ONLY");
  const [items, setItems] = useState<TargetPortfolioItem[]>(
  ...
  function updateItem(idx: number, patch: Partial<TargetPortfolioItem>) {
