...
export default function RebalancingTradePlanPanel({
  ...
  return (
    <div className="rounded-xl bg-gray-100 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700/50 p-3 space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-xs font-medium text-gray-700 dark:text-gray-300">종목별 거래 계획</div>
        <button
          onClick={() => setShowTradePlan((v) => !v)}
          className="sm:hidden text-xs text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
      ...
      <div className={`sm:hidden space-y-2 ${showTradePlan ? "block" : "hidden"}`}>
        {sellItems.length > 0 && (
          <div className="space-y-1.5">
            <div className="text-xs text-blue-600 dark:text-blue-400 font-medium">매도</div>
            ...
            {sellItems.length > 1 && (
              <div className="flex justify-between text-xs pt-1 border-t border-gray-200 dark:border-gray-700/40">
                <span className="text-gray-500 dark:text-gray-500">매도 합계</span>
        ...
        {buyItems.length > 0 && (
          <div className="space-y-1.5">
            <div className="text-xs text-green-600 dark:text-green-400 font-medium">매수</div>
            ...
            {buyItems.length > 1 && (
              <div className="flex justify-between text-xs pt-1 border-t border-gray-200 dark:border-gray-700/40">
                <span className="text-gray-500 dark:text-gray-500">매수 합계</span>
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700/50 text-gray-500 dark:text-gray-500">
            ...
            {sellItems.length > 0 && (
              <>
                <tr>
                  <td
                    colSpan={5}
                    className="pt-2 pb-1 text-blue-600 dark:text-blue-400 font-medium"
                  >
                    매도
                  </td>
                </tr>
                {sellItems.map((item, idx) => (
                  <tr key={idx} className="border-b border-gray-100 dark:border-gray-700/30">
                    <td className="py-1.5 pr-3 text-gray-800 dark:text-gray-200 truncate max-w-[140px]">
                      {item.name}
                    </td>
                    <td className="py-1.5 px-3 text-right text-gray-500 dark:text-gray-400">
                      {fmtKrw(item.current_price_krw!)}
                    </td>
                    <td className="py-1.5 px-3 text-right text-gray-800 dark:text-gray-200">
                      {Math.abs(Math.round(item.shares_to_trade!))}주
                    </td>
                    <td className={`py-1.5 px-3 text-right font-medium ${LOSS_COLOR}`}>
                      {fmtKrw(calcTradeKrw(item))}
                    </td>
                    <td className="py-1.5 pl-3 text-right text-gray-500 dark:text-gray-500">
            ...
            {buyItems.length > 0 && (
              <>
                <tr>
                  <td
                    colSpan={5}
                    className="pt-2 pb-1 text-green-600 dark:text-green-400 font-medium"
                  >
                    매수
                  </td>
                </tr>
      ...
      {/* 예상 거래 비용 합계 */}
      {estFee > 0 && (
        <div className="flex items-center justify-between text-xs border-t border-gray-200 dark:border-gray-700/40 pt-2.5">
      ...
      {unpricedItems.length > 0 && (
        <div className="text-xs text-amber-600 dark:text-amber-500/80 pt-1">
