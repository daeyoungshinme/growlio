...
export default function PortfolioListSection({
  ...
  function handleToggleTarget(e: React.MouseEvent, p: Portfolio) {
    ...
    if (currentState === "full") {
      onBatchSetTarget(null, relevant.map((a) => a.id));
      return;
    }
    ...
    if (conflicting.length > 0) {
      const conflictNames = conflicting
        .map((a) => {
          const targetPName = portfolios.find((po) => po.id === a.target_portfolio_id)?.name ?? "다른 포트폴리오";
    ...
    onBatchSetTarget(p.id, relevant.map((a) => a.id));
  }
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-1">
                            ...
                            <div className="flex gap-0.5 shrink-0 items-center">
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); onOpenAlertModal(p.id); }}
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); onConfirmDelete(p.id); }}
                                aria-label="포트폴리오 삭제"
